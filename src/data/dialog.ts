import { MessageObject } from "../types/message";

export const dialogTest: MessageObject[] = [
  {
    role: "system",
    content: "Системное сообщение. Оно необязательно и поддерживает markdown",
  },
  {
    role: "user",
    content: "Некий вопрос пользователя",
  },
  {
    role: "assistant",
    content:
      "Ответ. Вопрос о творческом мастерстве исполнителя сказочного фольклора.\n А это новый абзац.",
  },
  {
    role: "user",
    content: "Привет",
  },
  {
    role: "assistant",
    content: "Ответ. Привет",
  },
  {
    role: "user",
    content:
      "Краткое описание задания. Оно необязательно. Поддерживает markdown.\n\nК примеру, вот ненумерованный список:\n- первый пункт списка\n- второй пункт списка\n\nА вот пример нумерованного списка:\n1. Первый пункт списка\n2. Второй пункт списка",
  },
  {
    role: "assistant",
    content: "Ответ. Я не смог распарсить ваш вопрос.",
  },

  {
    role: "user",
    content: "Дай флуд",
  },
  {
    role: "assistant",
    content:
      "В сказковедении ставится проблема авантекста, разрешение которой решает вопрос о творческом мастерстве исполнителя сказочного фольклора, ведь он не запоминает текст сказки целиком, а порождает текст на глазах у слушателей, восстанавливая конструктивные элементы текста, — тематические (мотивы) и стилистические («общие места», формулы и пр.). Сказитель хранил в памяти сказочные сюжеты целиком или в форме единиц сюжета, т. н. мотива, и воспроизводил в исполняемой сказке. Сказковедение перечислило все обнаруженные сказочные сюжеты, собрав их в составленные указатели. Некоторые сказочные сюжеты встречаются в одном тексте (контаминация сюжетов). Сказитель, для придания эпического замедления, использовал приём утроения действия в сказке. В сказковедении идёт постоянный поиск метода полного описания структуры текста фольклорной сказки.",
  },
];
